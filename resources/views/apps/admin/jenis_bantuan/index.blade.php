        ...
        <div class="row flex-grow">
          <div class="col-12 grid-margin stretch-card">
            <div class="card card-rounded">
              <div class="card-body">
                <div>
                  <h3 class="card-title card-title-dash">Jenis Bantuan</h3>
                </div>
                <p></p>
                <div class="d-flex justify-content-between align-items-start">
                  <a href="{{ route('admin.jenis_bantuan.create') }}">
                    <button class="btn btn-primary btn-sm text-white mb-0 me-0" type="button"><i class="mdi mdi-plus"></i>Tambah</button>
                  </a>
                  <form action="{{ route('admin.jenis_bantuan') }}" method="GET">
                ...
                <div class="table-responsive mt-1">
                  <table class="table">
                    ...
                    <tbody>
                        @if (!empty($q_name))
                          @if (count($jenis_bantuan) == 0)
                            <tr>
                              <td colspan="3">Data tidak ditemukan.</td>
                            </tr>
                          @endif
                        @elseif(count($jenis_bantuan) == 0)
                          <tr>
                        ...
                        @foreach ($jenis_bantuan as $item)
                            <tr>
                                ...
                                <td width="30%">
                                    <a href="{{ route('admin.jenis_bantuan.edit', $item->id) }}" style="text-decoration: none"> 
                                        <button class="btn btn-warning btn-sm text-white mb-0 me-0" type="button"><i class="mdi mdi-pencil"></i>Edit</button>
                                    </a>
                                    <form action="{{ route('admin.jenis_bantuan.delete') }}" style="display: inline-block" method="POST">
                                        @csrf @method('DELETE')
