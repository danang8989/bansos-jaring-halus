        ...
        <div class="row flex-grow">
          <div class="col-12 grid-margin stretch-card">
            <div class="card card-rounded">
              <div class="card-body">
                <div>
                  <h3 class="card-title card-title-dash">Klasifikasi Konsumsi</h3>
                </div>
                <p></p>
                <div class="d-flex justify-content-between align-items-start">
                  <a href="{{ route('admin.konsumsi.create') }}">
                    <button class="btn btn-primary btn-sm text-white mb-0 me-0" type="button"><i class="mdi mdi-plus"></i>Tambah</button>
                  </a>
                  <form action="{{ route('admin.konsumsi') }}" style="display: flex;" method="GET">
                    ...
                    <tbody>
                        ...
                        @foreach ($konsumsi as $item)
                            <tr>
                              ...
                              <td>{{ $item->frekuensi_perminggu }}</td>
                              <td>{{ $item->biaya_pengobatan }}</td>
                                <td width="30%">
                                    <a href="{{ route('admin.konsumsi.edit', $item->id) }}" style="text-decoration: none"> 
                                        <button class="btn btn-warning btn-sm text-white mb-0 me-0" type="button"><i class="mdi mdi-pencil"></i>Edit</button>
                                    </a>
                                    <form action="{{ route('admin.konsumsi.delete') }}" style="display: inline-block" method="POST">
                                        @csrf @method('DELETE')
